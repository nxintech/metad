...
func (m *Metad) initRouter() {
	...
	m.router.HandleFunc("/map", m.handleWrapper(m.mapHandler)).Methods("GET", "HEAD")
	m.router.HandleFunc("/map/{nodePath:.*}", m.handleWrapper(m.mapHandler)).
		Methods("GET", "HEAD")
...
func (m *Metad) mapHandler(ctx context.Context, req *http.Request) (currentVersion int64, result interface{}, httpErr *HttpError) {
	...
	if wait {
		...
		// if prevVersion > currentVersion, may be metad reboot and recount version, so return immediately, let client use new version.
		if prevVersion > 0 && prevVersion != currentVersion {
			result = m.metadataRepo.Map(clientIP, key, nodePath)
		} else {
			m.metadataRepo.WatchSelf(ctx, clientIP, nodePath)
			// directly return new result to client ,not change, for pre_version.
			result = m.metadataRepo.Map(clientIP, key, nodePath)
	...
	if result == nil {
		httpErr = NewHttpError(http.StatusNotFound, "Not found map data of key")
	}
	return
}
